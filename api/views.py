...
from .models import Politician, Poll, PollResult, Vote
from .serializers import (PoliticianSerializer, PollSerializer,
                          ResultSerializer, VoteSerializer)
...
class PoliticianViewSet(viewsets.ModelViewSet):
    ...
    def votes(self, request, pk):
        ...
        serializer = VoteSerializer(
            data=vote_list, many=True, context={'politician': pk})
...
class PollViewSet(viewsets.ModelViewSet):
    ...
    def get_result(self, abgeordnetenwatch_id):
        r = requests.get(
            f'https://www.abgeordnetenwatch.de/api/v2/votes?poll={abgeordnetenwatch_id}&range_end=999')
        votes = r.json()['data']
    ...
    def list(self, request):
        ...
        r = requests.get(
            f'https://www.abgeordnetenwatch.de/api/v2/polls?page={page_num}&pager_limit={page_size}')
        ...
        for poll in response:
            ...
            try:
                ...
                serializer = PollSerializer(
                    saved_poll, context={'request': request})
                ...
                serializer = PollSerializer(
                    data=new_poll, context={'request': request})
                if serializer.is_valid():
                    serializer.save()
